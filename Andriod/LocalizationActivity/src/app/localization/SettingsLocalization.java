...
/**
 * Settings page. If the settings page uses the Android built-in settings page,
...
public class SettingsLocalization extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// how to get shared preferences in case settings is an actual Android
		// setting
		/*
		 ...
		 * 
		 * builder.append("\nEnable localization: " +
		 * sharedPrefs.getBoolean("perform_localization", false));
		 * builder.append("\nOpt out of all targeted deals: " +
		 * sharedPrefs.getBoolean("opt_out_all", false));
		 * builder.append("\nSpecific merchants: " +
		 * sharedPrefs.getString("specific_merchants", "NULL"));
		 * 
		...
		checkAllButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				for (int i = 0; i < myListView.getAdapter().getCount(); i++) {
					...
					Settings.optInOut(SettingsLocalization.this,
							CommonUtilities.OPTINALL_URL);
	...
	/**
	 * Retrieve a list of currently subscribed merchants.
	 */
	public void getMerchants() {

		String username = CommonUtilities
				.getUsername(SettingsLocalization.this);
		...
		final JSONArray jsonArray = RestClient.connectToDatabase(
				CommonUtilities.ALLMERCHANTS_URL, null);
		final JSONArray subscribedJsonArray = RestClient.connectToDatabase(
				CommonUtilities.ALLSUBSCRIBEDMERCHANTS_URL, jsonIn);
				...
				public void run() {
					...
					try {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							String merchantName = jsonArray.getJSONObject(i)
									.getString("userName");
					...
					try {
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							Log.i("TAG", "SUBSCRIBED JSON = "
									+ subscribedJsonArray.toString());
							Log.i("TAG", "subscribedJsonArray length = "
									+ subscribedJsonArray.length());
							boolean found = false;
							for (int j = 0; j < subscribedJsonArray.length(); j++) {
								Log.i("TAG", "subscribed merchant = "
										+ subscribedJsonArray.getJSONObject(j)
												.getString("merchant"));
								if (merchantName
										.equals(subscribedJsonArray
						...
						Log.i("TAG",
								"Exception while adding checkbox default values");
						Log.i("TAG", e.toString());
					}

					// Each deal item can be clicked on and taken to another
					// page
					myListView
							.setOnItemClickListener(new OnItemClickListener() {
								...
								public void onItemClick(AdapterView<?> a,
									...
									String merchantName = adapter
											.getItem(position);

									checkBoxList.set(position,
											!checkBoxList.get(position));

									Log.i("TAG", merchantName + " is = "
											+ checkBoxList.get(position));
